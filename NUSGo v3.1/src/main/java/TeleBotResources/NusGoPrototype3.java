    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().hasText()) { // CHECK USER INPUT
            ...
            if (text_message.equals("/start") || text_message.equals("/restart")) { // HOME page
                ...
                if (searchStartLocationViaTextCounter > 1 && search_text_message.charAt(1) != '.') { // multiple suggestions
                    List<KeyboardRow> rowsInLine = new ArrayList<KeyboardRow>();
                ...
                } else if (searchStartLocationViaTextCounter == 1) { // single suggestion
                    prevPsrList = new ArrayList<PlacesSearchResult>(); // reset
                    prevPsrList.add(psrArr[0]); // record down current results
                } else if (search_text_message.substring(1, 3).equals(". ")) { // check if this is a user selection
                    prevPsrList_index = Integer.parseInt(search_text_message.substring(0, 1)) - 1;
                }

                if (page.equals(Page.SEARCH_LOCATION_START)) {
                ...
                String returnMsg = "This is not an NUS location."; // reply if location is not in NUS
