...
public class NUSGoBot extends TelegramLongPollingBot {
    ...
    ArrayList<PlacesSearchResult> cachedPsrList = new ArrayList<PlacesSearchResult>();
    int cachedPsrList_index = 0;
    ...
    public void refresh() {
        ...
        this.psrArr = null;
        this.cachedPsrList = new ArrayList<PlacesSearchResult>();
        this.cachedPsrList_index = 0;

        NUSGoBot.cachedRoute = null;
    }
        ...
        if (update.hasMessage() && update.getMessage().hasText()) { // CHECK USER INPUT
            ...
            if (text_message.equals("/start") || text_message.equals("/restart")) { // HOME page
                ...
                if (text_message.charAt(0) != '/') { // invalid command
                    ...
                    venue = new SendVenue().setChatId(chat_id).setTitle(startingBusStop.getSymbol())
                            .setAddress(startingBusStop.getName())
                            .setLatitude((float) startingBusStop.getLatLong().lat)
                            .setLongitude((float) startingBusStop.getLatLong().lng);
                }
                ...
                if (text_message.charAt(0) != '/') { // invalid command
                    ...
                    String returnMsg = "Here is how you get from " + startingBusStop.getName() + " Bus Stop to "
                            + destinationBusStop.getName() + " Bus Stop: ";
                    for (Route r : finalR) returnMsg += "\n\n" + r.toString();
                ...
                if (searchStartLocationViaTextCounter >= 1) { // suggestions avaliable
                    ...
                    cachedPsrList = new ArrayList<PlacesSearchResult>(); // reset
                    ...
                    for (PlacesSearchResult psr : psrArr) {
                        ...
                        if (psrLat < 1.292331 || psrLng < 103.767406 || psrLat > 1.306875 || psrLng > 103.786586) {
                            ...
                            cachedPsrList.add(psr); // record down current results
                        }
                    }
                    ...
                    if (cachedPsrList.isEmpty()) { // valid local but invalid NUS
                ...
                if (text_message.substring(1, 3).equals(". ")) { // check if this is a user selection
                    cachedPsrList_index = Integer.parseInt(text_message.substring(0, 1)) - 1;
                    if (page.equals(Page.SEARCH_LOCATION_START_UPDATE)) {
                        page = Page.SEARCH_LOCATION_END;
                        startingLocation = new Building(cachedPsrList.get(cachedPsrList_index)); //Assigning starting building
                        returnMsg = "Your starting point is " + cachedPsrList.get(cachedPsrList_index).name + ". Please enter your destination location.";
                    } else {
                        destinationLocation = new Building(cachedPsrList.get(cachedPsrList_index));
                        returnMsg = "Your destination is " + cachedPsrList.get(cachedPsrList_index).name +
                                ".\n\nHere is how you get from " + startingLocation.getName() + " to " + destinationLocation.getName();
            ...
            try {
                System.out.println("excuted at 425");
                execute(message);
                execute(venue);
                execute(destination_request_message);
            } catch (TelegramApiException e) {
