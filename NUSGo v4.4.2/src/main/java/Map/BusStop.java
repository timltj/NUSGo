...
//    }
    ...
    public void assignNeighbours() {
        ...
        busStopBuilding.getBusStopLst().removeIf(x -> {
            ...
            if (duration <= 120) {
                this.directionsTable.put(x.getSymbol(), new Directions(this.getSymbol() + " BusStop"
                        , x.getSymbol() + " Bus Stop", duration));
    ...
    public ArrayList<Route> useOnCrossBusStops(BusStop destinationBusStop){
        ...
        if (commonBusService != null) {
            ...
            Route curr = new Route(returnRoute, commonBusService.getDurationInSeconds());
            // curr.addWaitingDuration(commonBusService.getDurationInSeconds());
                ...
                for (String Service : neighbour.ServiceOrder.keySet()) {
                    ...
                    if (destinationBusStop.ServiceOrder.containsKey(Service)) {
                        if (Service.equals("CC") || destinationBusStop.ServiceOrder.get(Service) > neighbour.ServiceOrder.get(Service)) {
                            ...
                            double totalDuration = dir.getDurationInSeconds()
                                    + dirNeigh.getDurationInSeconds();

                            Route curr = new Route(returnRoute, totalDuration);
                            curr.addWaitingDuration(extraWaitingTime);
    ...
    public ArrayList<Route> findRoute(BusStop destinationBusStop) {
        ...
        for (String Service : destinationBusStop.ServiceOrder.keySet()) {
            ...
            for (BusStop bs : stred) { //for every busstop that is a crossable
                ...
                currentRouteList.forEach(x -> {
                    Directions dir = this.getDirectionsTable().get(bs.getSymbol()); //getting direction to crossable
