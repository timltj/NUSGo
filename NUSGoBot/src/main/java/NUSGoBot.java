        ...
        if (update.hasMessage() && update.getMessage().hasText()) { // CHECK USER INPUT
            ...
            if (text_message.equals("/start") || text_message.equals("/restart")) { // command - start NUSGoBot
                ...
                if (!text_message.contains("/") /* && valid from database */) { // invalid command
                    ...
                    message = new SendMessage()
                            .setChatId(chat_id)
                            .setText("Your starting bus stop is: " + text_message.substring(1));
                    ...
                    message = new SendMessage()
                            .setChatId(chat_id)
                            .setText("Your destination bus stop is: " + text_message.substring(1));
                    venue = new SendVenue().setChatId(chat_id).setTitle("Name Here").setAddress("Address Here")
                            .setLatitude(1.294260f)
                            .setLongitude(103.773806f);
                    route_message = new SendMessage()
                            .setChatId(chat_id)
                            .setText("For simulation's sake, take Bus A2 from your current location " +
                                    "to reach your destination bus stop " + text_message.substring(1) + ".");
                }
            }
            ...
            venue = new SendVenue().setChatId(chat_id).setTitle("COM 2").setAddress("15 Computing Dr, Singapore 117418")
                    .setLatitude(1.294260f)
                    .setLongitude(103.773806f);
            destination_request_message = new SendMessage()
                    .setChatId(chat_id)
