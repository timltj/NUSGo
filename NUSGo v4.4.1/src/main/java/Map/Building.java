...
public class Building {
    ...
    public ArrayList<Route> findRoute(Building destination) {
        System.out.println("line 58: Building.findRoute() called");
        ...
        if (!commonAvailable) {
            ...
            for (int i = 0; i < 3; i++) {
                Directions walkToStartingBusStop = new Directions(this.name, busStopsFromOrigin.get(i).getName() + " BusStop", walkToStartBusStopDistanceMatrix[i].rows[0].elements[0].duration.inSeconds);
                for (int j = 0; j < 3; j++) {
                    ArrayList<Route> supplementRoutes = new ArrayList<>();
                    Directions walkFromEndingBusStop = new Directions(busStopsFromDestination.get(j).getName() + " BusStop", destination.getName(), walkToStartBusStopDistanceMatrix[i].rows[0].elements[0].duration.inSeconds);
                    ...
                    supplementRoutes.forEach(x -> { //add the walking time and directions
                        x.addDuration(walkToStartingBusStop.getDurationInSeconds());
                        x.addDuration(walkFromEndingBusStop.getDurationInSeconds());
                        x.getDirectionsArrayList().add(0, walkToStartingBusStop);
                        x.getDirectionsArrayList().add(walkFromEndingBusStop);
                    });
        ...
        System.out.println("final route returned");
        return finalRoutes;
    }
